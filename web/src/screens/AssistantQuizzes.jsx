...
export default function AssistantQuizzes() {
  ...
  const [form, setForm] = useState({ title: '', course_code: '', auditorium: '', duration: 20 })
  ...
  const createItem = async () => {
    ...
    setForm({ title: '', course_code: '', auditorium: '', duration: 20 })
    await load()
    toast.push({ title: 'Quiz créé' })
  }
  const del = async (id) => { 
  ...
  return (
    <div className="grid gap-4">
      <div className="card p-4 grid gap-2">
        <h3 className="text-lg font-semibold">Nouveau Quiz</h3>
        <div className="grid md:grid-cols-2 gap-3">
          <input className="px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800" placeholder="Titre" value={form.title} onChange={e=>setForm({...form,title:e.target.value})} />
          ...
          <input type="number" min="5" className="px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800" placeholder="Durée (min)" value={form.duration} onChange={e=>setForm({...form,duration:Number(e.target.value)})} />
        </div>
        <div>
          <button className="btn" onClick={createItem}>Créer</button>
        </div>
      </div>
      ...
      <div className="card p-4 overflow-auto">
        ...
        <table className="min-w-full text-sm">
          <thead><tr className="text-left text-slate-500"><th className="py-2 pr-4">Titre</th><th className="py-2 pr-4">Cours</th><th className="py-2 pr-4">Auditoire</th><th className="py-2 pr-4">Durée</th><th className="py-2 pr-4"></th></tr></thead>
          <tbody>
            {rows.map(r => (
