    ...
    <div className="grid gap-3">
      <div className="flex gap-2">
        <select className="px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800" value={aud} onChange={e => setAud(e.target.value)}>
          {auditoriums.map(a => <option key={a.code} value={a.code}>{a.code} • {a.name}</option>)}
        </select>
      ...
      <div className="card p-4 overflow-auto">
        ...
        <tbody>
          {rows.map(r => (
            <tr key={r.student_id} className="border-t border-slate-200/60 dark:border-slate-800/60">
