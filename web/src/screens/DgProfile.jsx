...
export default function DgProfile() {
  // Dummy profile data for Directeur Général
  const [profile, setProfile] = useState({
    prenom: 'Marie',
    nom: 'Curie',
    email: 'marie.curie@univ.com',
    phone: '+243 991 234 567',
    address: '456 Boulevard de l\'Indépendance, Kinshasa',
    avatar: 'https://i.pravatar.cc/150?u=marie.curie@univ.com',
    role: 'Directeur Général',
    dateNomination: '01/01/2020',
    mandat: '5 ans',
    departementOrigine: 'Physique',
  });
  ...
  const handleProfileUpdate = async (e) => {
    e.preventDefault();
    // Simulate API call
    console.log("Updating DG profile with:", formData, profilePictureFile);
    toast.push({ title: 'Informations mises à jour', message: 'Votre profil DG a été sauvegardé (simulation).', kind: 'success' });
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    // Simulate API call
    console.log("Updating DG password with:", passwordData);
    toast.push({ title: 'Mot de passe mis à jour', message: 'Votre mot de passe DG a été changé (simulation).', kind: 'success' });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-1">
        ...
        <div className="card bg-white dark:bg-slate-800 p-6 shadow-lg mt-8">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Informations du Directeur Général</h3>
          <div className="grid grid-cols-1 gap-y-2 text-sm">
            ...
            <div className="flex justify-between py-1 border-b border-slate-200 dark:border-slate-700">
              ...
              <span className="font-medium text-slate-700 dark:text-slate-300">{profile.dateNomination}</span>
            </div>
            <div className="flex justify-between py-1 border-b border-slate-200 dark:border-slate-700">
              <span className="text-slate-500 dark:text-slate-400">Mandat:</span>
              <span className="font-medium text-slate-700 dark:text-slate-300">{profile.mandat}</span>
            </div>
            <div className="flex justify-between py-1">
              <span className="text-slate-500 dark:text-slate-400">Département d'origine:</span>
              <span className="font-medium text-slate-700 dark:text-slate-300">{profile.departementOrigine}</span>
            </div>
          </div>
        </div>
      </div>
