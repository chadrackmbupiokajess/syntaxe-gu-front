...
export default function AssistantNewTPTD() {
  ...
  useEffect(() => {
    axios.get(`/api/assistant/auditoriums/${encodeURIComponent(code)}/courses`).then(r => {
      setCourses(r.data || []);
      if (r.data.length > 0) {
        setForm(f => ({ ...f, course_id: r.data[0].id }));
      }
    });
  }, [code]);
  ...
  const addQuestion = () => {
    setForm({ ...form, questions: [...form.questions, { text: '', type: 'text', choices: [] }] }); // Default to text for TP/TD
  };
  ...
  const submitTPTD = async () => {
    if (!form.course_id || !form.title || !form.deadline) {
      toast.push({ kind: 'error', title: 'Champs requis', message: 'Cours, titre et deadline sont requis.' });
      return;
    }
    setSaving(true);
    try {
      ...
      toast.push({ title: 'Succès', message: 'TP/TD créé avec succès.' });
    } catch (error) {
      console.error("Error creating TPTD:", error);
      toast.push({ kind: 'error', title: 'Erreur', message: error.response?.data?.detail || 'Impossible de créer le TP/TD.' });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-1 space-y-6">
        <div className="card bg-white dark:bg-slate-800 p-5 shadow-lg">
          <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-4">Paramètres du TP/TD</h2>
          <div className="space-y-4">
            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300">Cours
              <select value={form.course_id} onChange={e => setForm({ ...form, course_id: e.target.value })} className="mt-1 w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800">
                <option value="">— Choisir —</option>
                {courses.map(c => (<option key={c.id} value={c.id}>{c.title} ({c.code})</option>))}
              </select>
            </label>
            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300">Titre
              <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="mt-1 w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800" placeholder="Ex: Exercices sur les boucles" />
            </label>
            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300">Deadline (ISO 8601)
              <input type="datetime-local" value={form.deadline} onChange={e => setForm({ ...form, deadline: e.target.value })} className="mt-1 w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800" />
            </label>
            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300">Points Totaux
              <input type="number" min="0" value={form.total_points} onChange={e => setForm({ ...form, total_points: Number(e.target.value) })} className="mt-1 w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800" />
            </label>
          </div>
        </div>
        <div className="flex flex-col gap-3">
          <button className="btn btn-lg w-full" onClick={submitTPTD} disabled={saving || !form.course_id || !form.title || !form.deadline}>
            {saving ? 'Enregistrement...' : 'Enregistrer le TP/TD'}
          </button>
          <button className="btn btn-lg w-full !bg-slate-600" onClick={() => window.history.back()}>Annuler</button>
        </div>
      </div>
      ...
      <div className="lg:col-span-2 space-y-4">
        <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold text-slate-800 dark:text-white">Questions du TP/TD</h2>
