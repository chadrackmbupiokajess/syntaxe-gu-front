...
export default function AssistantNewTPTD(){
  ...
  useEffect(()=>{ axios.get(`/api/assistant/auditoriums/${encodeURIComponent(code)}/courses`).then(r=>setCourses(r.data||[])) },[code])

  const submit = async ()=>{
    if(!courseId || !title || !deadline) return
    setSaving(true)
    try{
      const { data } = await axios.post(`/api/assistant/auditoriums/${encodeURIComponent(code)}/tptd/new`, { course_id:Number(courseId), title, deadline })
      setCreated(data)
  ...
  return (
    <div className="grid gap-4 min-h-screen bg-slate-900 text-white p-4">
      ...
      <div className="card p-4 bg-slate-800">
        ...
        <label className="text-sm block mt-3">Deadline (ISO 8601)
